...
class OpenAIProvider implements LLMProvider {
  ...
  async call(params: LLMCallParams): Promise<LLMCallResult> {
    ...
    if (!content) {
      // Check for reasoning model (content empty but reasoning present)
      const reasoning = data.choices?.[0]?.message?.reasoning;
      if (reasoning) {
        throw new Error("Empty content (reasoning model detected - content in reasoning field not usable as JSON)");
      }
...
// Module-level cache state (persists for the lifetime of the process)
...
class GeminiProvider implements LLMProvider {
  ...
  constructor(
    ...
    this.cacheTtl = parseInt(process.env.LLM_CACHE_TTL ?? "3600", 10);
  }
  ...
  private async ensureCache(systemPrompt: string): Promise<string | null> {
    ...
    try {
      ...
      cachedSystemPrompt = systemPrompt;
      console.error(`Gemini context cache created: ${cachedContentName}`);
      return cachedContentName;
    } catch (err) {
